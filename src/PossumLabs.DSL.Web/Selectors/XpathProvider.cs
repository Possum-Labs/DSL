...
namespace PossumLabs.DSL.Web.Selectors
{
    public class XpathProvider : IXpathProvider
    {
        ...
        //matches
        // assuming we match on "First Name" we will match the following
        //First Name
        //First Name:
        virtual public string TextMatch(string target)
            => $"text()[" +
            $"normalize-space(.)={target.XpathEncode()} or " +
            $"normalize-space(.)={$"{target}:".XpathEncode()}]";
        ...
        virtual public string ClickableElements
          => "(not(@type='hidden') and ( self::a or self::button or self::input or @role='button' or @role='link' or " +
            "@role='menuitem' ))";
